...
define view DCM_PROJECT_{{db}}.ANALYTICS.V_ENRICHED_ORDER_DETAILS
comment = 'I want to be a dynamic table when I grow up'
as
select
    oh.ORDER_ID,
    oh.ORDER_TS,
...
define view DCM_PROJECT_{{db}}.ANALYTICS.V_MENU_ITEM_POPULARITY
as
select
    MENU_ITEM_NAME,
    ITEM_CATEGORY,
...
define view DCM_PROJECT_{{db}}.ANALYTICS.V_CUSTOMER_SPENDING_SUMMARY
as
select
    CUSTOMER_ID,
    FIRST_NAME,
...
define view DCM_PROJECT_{{db}}.ANALYTICS.V_TRUCK_PERFORMANCE
as
select
    TRUCK_BRAND_NAME,
    COUNT(DISTINCT ORDER_ID) AS TOTAL_ORDERS,
